...
# fare l else che metta il messaggio ObjDet = false
		...
		ballState.ballDetected = true 
		ballState.vel_angular.z = -0.002*(center[0]-400)
                self.vel_pub.publish(ballState)

        else:
	    ballState = Ball_state()
            ballState.ballDetected = false
	    ballState.vel_angular_z = 0
	    ballState.vel_lin_x = 0
