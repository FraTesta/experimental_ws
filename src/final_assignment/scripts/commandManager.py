...
def newRoomDetected(color):
	...
	rospy.loginfo("[CommandManager] New %s ball detected!!! ", color.data)
	#NEW_ROOM = True
	# stop detecting 
	roomD_pub.publish(False)
	...
	roomD_pub.publish(True)
	    ...
	    time.sleep(2)
            if control_variables["PLAY"] == True:
                ...
                rospy.loginfo("[CommandManager] Max number of Normal state iteration")
                self.counter = 0
                return 'goToSleep' 
...
class Track(smach.State):
	...
	rospy.loginfo("[CommandManager] I'm in TRACK state")
	    ...
	    if result.x != 0 and result.y != 0:
		 ...
		 if control_variables["FIND_MODE"]:
		    control_variables["NEW_ROOM_COLOR"] = "None"
		    return "goToFind"
            control_variables["NEW_ROOM_COLOR"] = "None"
            return "goToNormal"
...
class Find(smach.State):
	...
	roomD_pub.publish(True)
	    ...
	    if control_variables["NEW_ROOM_COLOR"] != "None":
                rooms.cancel_last_visited_location()
                return 'goToTrack'
            elif self.counter == 5:
